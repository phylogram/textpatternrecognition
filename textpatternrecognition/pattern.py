...
class PotentialPattern:
    ...
    def __init__(self, current_line_number: int or None = None, pattern_line_number: int or None = None,
                 position: int or None = None, completed: bool or None = None):
...
class PatternParser:
    ...
    def _evaluate_search_objects(self, text: str, pattern_line_number: int, position: int) -> bool or None:
        """
        Method that evaluates text + searchobject, for example re.compile('a').match('a')
        ...
        if pattern_line_number is self.number_of_search_pattern_lines and match_object:
            ...
            raise Exception("Error in Logic. Please go debugging")  # No clue what is going on – debug!
    ...
    def _continue_parsing_potential_objects(self) -> int:
        """
